 ...
 */
export function migrateFileConfigToDb(dataDir) {
    if (hasSettings())
    ...
    if (source.HA_URL || source.ha_url)
        entries.HA_URL = str(source.HA_URL || source.ha_url);
    if (source.HA_TOKEN || source.ha_token)
        entries.HA_TOKEN = str(source.HA_TOKEN || source.ha_token);
    if (source.PORT || source.port)
        entries.PORT = str(source.PORT || source.port);
    if (source.LOG_LEVEL || source.log_level)
        entries.LOG_LEVEL = str(source.LOG_LEVEL || source.log_level);
