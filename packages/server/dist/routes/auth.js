...
import { getAdminPassword, verifyPassword, setAdminPassword, } from '../db.js';
...
router.get('/config', requireAdmin, (_req, res) => {
    const config = getConfig();
    const missing = [];
    if (!config.HA_URL || config.HA_URL === 'http://localhost:8123')
        missing.push('HA_URL');
    if (!config.HA_TOKEN)
        missing.push('HA_TOKEN');
    res.json({
        success: true,
        config: {
...
router.post('/config', requireAdmin, (req, res) => {
    ...
    try {
        ...
        if (logLevel !== undefined)
            updates.LOG_LEVEL = String(logLevel).toUpperCase();
        if (allowedEntities !== undefined) {
            updates.ALLOWED_ENTITIES = typeof allowedEntities === 'string'
                ? allowedEntities.split(',').map((e) => e.trim()).filter(Boolean)
                : allowedEntities;
        }
...
// Restart server
router.post('/restart', requireAdmin, (_req, res) => {
