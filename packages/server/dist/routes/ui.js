...
import { readHiddenEntities, setHiddenEntities, setEntityHidden, readLayout, saveLayout, readGuestEntities, readGuestTitle, saveGuestEntities, saveGuestTitle, readModules, createModule, updateModule, deleteModule, setModuleEntities, reorderModules, readAllEntityDisplayProps, readEntityDisplayProps, saveEntityDisplayProps, deleteEntityDisplayProps, } from '../db.js';
const router = Router();
// ─── Hidden entities ────────────────────────────────────────
router.get('/hidden', async (_req, res) => {
    res.json({ hidden: readHiddenEntities() });
});
router.post('/hidden', requireAdmin, (req, res) => {
    const hidden = Array.isArray(req.body?.hidden) ? req.body.hidden : [];
    setHiddenEntities(hidden);
    res.json({ hidden });
});
router.post('/hidden/:entityId', requireAdmin, (req, res) => {
...
router.post('/layout', requireAdmin, (req, res) => {
    const order = Array.isArray(req.body?.order) ? req.body.order : [];
    saveLayout(order);
    res.json({ order });
});
// ─── Guest entities & title ─────────────────────────────────
router.get('/guest-entities', async (_req, res) => {
    res.json({ entities: readGuestEntities(), title: readGuestTitle() });
});
router.post('/guest-entities', requireAdmin, (req, res) => {
...
router.post('/modules/:id/entities', requireAdmin, (req, res) => {
    const id = Number(req.params.id);
    const entities = Array.isArray(req.body?.entities) ? req.body.entities : [];
    setModuleEntities(id, entities);
    res.json({ success: true, entities });
});
router.post('/modules/reorder', requireAdmin, (req, res) => {
...
router.get('/guest-modules', requireGuestPin, async (_req, res) => {
...
router.get('/guest-entity-props', requireGuestPin, (_req, res) => {
    res.json({ props: readAllEntityDisplayProps() });
});
// ─── Entity display props ───────────────────────────────────
router.get('/entity-props', requireAdmin, (_req, res) => {
    res.json({ props: readAllEntityDisplayProps() });
});
router.get('/entity-props/:entityId', requireAdmin, (req, res) => {
...
router.post('/entity-props/:entityId', requireAdmin, (req, res) => {
    const entityId = req.params.entityId;
...
router.delete('/entity-props/:entityId', requireAdmin, (req, res) => {
