...
import { needsSetup, getConfig, saveConfig, loadConfig } from '../config.js';
import { setAdminPassword, setGuestPinEnabled, saveGuestPins } from '../db.js';
...
router.post('/configure', async (req, res) => {
    ...
    if (Array.isArray(pins)) {
        const validPins = pins.map((p) => String(p).trim()).filter((p) => p.length > 0);
